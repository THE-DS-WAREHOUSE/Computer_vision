# -*- coding: utf-8 -*-
"""
Created on Thu May 18 02:54:49 2023
...
DATA_DIR = 'data_dos_manos' # where we save the images collected for one-hand sign
...
for dir_ in os.listdir(DATA_DIR): # iterate over every folder and image
    for img_path in os.listdir(os.path.join(DATA_DIR, dir_)): # get the image
        ...
        if results.multi_hand_landmarks: # if we detect something then
            ...
            if len(data_aux) > 42: # this extra step is because we are only interest in images that had detected two hads, not just one
                data.append(data_aux) # save coordinates as a vector
                labels.append(dir_) # add label to data


f = open('data_dos_manos.pickle', 'wb') # write data as binary data
